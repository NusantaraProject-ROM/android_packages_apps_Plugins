...
package co.potatoproject.plugin.volume.jr;
...
import co.potatoproject.plugin.volume.jr.R;
...
public class VolumeDialogImpl extends PanelSideAware implements VolumeDialog {
    ...
    private void initDialog() {
        ...
        mDialog = LayoutInflater.from(mContext).inflate(R.layout.volume_dialog_jr,
        ...
        ringerLP.gravity = Gravity.CENTER;
        dummyButtonLP.gravity = Gravity.CENTER;
        mediaOutputLP.gravity = Gravity.CENTER;
        expandRowsLP.gravity = Gravity.CENTER;
        captionsLP.gravity = Gravity.CENTER;
    ...
    private void initAppRow(final VolumeRow row, final AppTrackData data) {
        row.view = LayoutInflater.from(mContext).inflate(R.layout.volume_dialog_jr_row, null);
    ...
    private void showH(int reason) {
        ...
        if(!mShowing && !mDialog.isShown()) {
            if (!isLandscape()) mDialogView.setTranslationX((mDialogView.getWidth() / 2.0f)*1);
            mDialogView.setAlpha(0);
            mDialogView.animate()
                    .alpha(1)
                    .translationX(0)
    ...
    protected void dismissH(int reason) {
        ...
        if (!isLandscape()) animator.translationX((mDialogView.getWidth() / 2.0f)*1);
        animator.start();
